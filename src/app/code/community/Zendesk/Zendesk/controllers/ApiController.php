...
class Zendesk_Zendesk_ApiController extends Mage_Core_Controller_Front_Action
{
    ...
    public function _authorise()
    {
        ...
        if (!$tokenString) {
            ...
            Mage::getSingleton('zendesk/logger')->debug('Unable to extract authorization header from request.');

            $this->getResponse()
                ->setBody(json_encode(array('success' => false,
                                            'message' => 'Unable to extract authorization header from request'
                )))
    ...
    public function customersAction()
    {
        ...
        $sections = explode('/', trim($this->getRequest()->getPathInfo(), '/'));
        $email    = trim(isset($sections[3]) ? $sections[3] : '');
        ...
        if (false === $validator->isValid($email)) {
            $this->getResponse()
                ->setBody(json_encode(array('success' => false, 'message' => 'Invalid Email Address')))
                ->setHttpResponseCode(404)
                ->setHeader('Content-type', 'application/json', true);
            return $this;
        }
        ...
        if ($customer && $customer->getId()) {
            $info = array(
                ...
                'admin_url' => $urlModel->getUrl('adminhtml/zendesk/redirect', array('id'   => $customer->getId(),
                                                                                     'type' => 'customer'
                )),
